    ...
    try {
        ...
        if (existingUser) {
            if(!existingUser.isVerified){
               return res.status(400).json({success:false,message:"Email already registered,Verify It",isVerified:false})
            }
...
async function profile(req,res) {
    try {
        ...
        const primaryaddress = userProfile.addresses.find(addr => addr.primary===true)
        
        res.status(200).json({success:true,userProfile,username:userProfile.username,address:primaryaddress})
...
async function profileUpdate(req,res) {
    ...
    const userId = req.user._id
    const updateprofile = await User.findByIdAndUpdate(userId,{username,phone,address},{new:true,runValidators:true})
    if(!updateprofile){
       return res.status(400).json({success:false,message:"User Not Found"})
    }
...
async function addNewAddress(req,res) {
    try {
      ...
      if(!addressLine || !city || !state || !postalCode){
        return res.status(400).json({success:false,message:"No address Details Provided"})
      }
      const user = await User.findById(userId)
      ...
      if(user.addresses.length>=3){
        res.status(400).json({success:false,message:"Only 3 Adddresses can create"})
      }
...
async function deleteAddress(req,res) {
    ...
    if(!addressId){
        return res.status(400).json({success:false,message:"AddressId id Required"})
    }
    const user = await User.findById(userId)
    if(!user){
        return res.status(400).json({success:false,message:"User not Found"})
    ...
    const isPrimary = user.addresses.find(
        addr => addr._id.toString()===addressId)?.primary
    ...
    await user.save()
    return res.status(200).json({success:true,message:"Deleted the address Successfully",user})
